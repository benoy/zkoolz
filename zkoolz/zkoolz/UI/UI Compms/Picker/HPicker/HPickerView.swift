...
class HPickerView: BaseView {
    ...
    override func commonInit() {
        Bundle.main.loadNibNamed(HPickerView.nibName , owner: self, options: nil)
        super.commonInit()
...
extension HPickerView: UICollectionViewDataSource {
...
extension HPickerView: UIScrollViewDelegate {
    
    func scrollViewWillBeginDragging(_ scrollView: UIScrollView) {
        let pageWidth = self.frame.size.width / 3
        let proportionalOffset = clctnView.contentOffset.x / pageWidth
    ...
    func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        ...
        if didUseSwipeToSkipCell {
            ...
            clctnView.scrollToItem(at: indexPath, at: .left, animated: true)
        }
    }
...
extension HPickerView: UICollectionViewDelegateFlowLayout {
    
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        let ht = self.frame.size.height
        let wdth = self.frame.size.width / 3
        return CGSize(width: wdth, height: ht)
    }
}


extension HPickerView: UICollectionViewDelegate {
